...
if (!constants_2.inE2ETests) {
    di_1.Container.get(n8n_core_1.Logger).error('E2E endpoints only allowed during E2E tests');
    process.exit(1);
}
...
let E2EController = E2EController_1 = class E2EController {
    constructor(license, settingsRepo, workflowRunner, mfaService, cacheService, push, passwordUtility, eventBus, userRepository, authUserRepository) {
    ...
    async truncateAll() {
        for (const table of tablesToTruncate) {
            try {
                ...
                di_1.Container.get(n8n_core_1.Logger).warn('Dropping Table for E2E Reset error', {
                    error: error,
                });
            }
        }
