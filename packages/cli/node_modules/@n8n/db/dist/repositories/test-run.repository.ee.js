...
let TestRunRepository = class TestRunRepository extends typeorm_1.Repository {
    ...
    async markAsCancelled(id, trx) {
        ...
        return await trx.update(entities_1.TestRun, id, { status: 'cancelled' });
    }
    ...
    async incrementPassed(id, trx) {
        trx = trx ?? this.manager;
        return await trx.increment(entities_1.TestRun, { id }, 'passedCases', 1);
    }
    async incrementFailed(id, trx) {
        trx = trx ?? this.manager;
        return await trx.increment(entities_1.TestRun, { id }, 'failedCases', 1);
    }
